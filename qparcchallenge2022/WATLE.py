...
def get_energy_EX(
    ...
    for i in range(loop_qubit):
        for j in range(loop_qubit):
            ...
            for pa in pata_XXU[i]:
                (a, b) = pa
                pair_syo[a] = a
                pair_syo[b] = a
            for pa in pata_XXD[j]:
                (a, b) = pa
                pair_syo[a] = a
                pair_syo[b] = a

            for k in range(ha_qubit):
                for h in range(k + 1, ha_qubit):
                    if pair_syo[k] > pair_syo[h]:
                        gen_cir.add_gate(CZ(k, h))
            for k in range(ha_qubit, n_qubit):
            ...
            pata_XXUD = pata_XXU[i] + pata_XXD[j]
